...
/**
 * Usart constructor which takes parameterized UsartConfig struct
 */
Usart::Usart(const UsartConfig& u_config): rx_buffer(u_config.rx_buffer_size), tx_buffer(u_config.tx_buffer_size) {
	setup_registers();
	usart0_bind_buffers(&rx_buffer, &tx_buffer);
	RX_FUNCTION_U0 = u_config.rx_action;
	uart_init(u_config.baud);
}

void Usart::setup_registers(){
